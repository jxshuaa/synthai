  ...
  try:
    ...
    def call_openai_api():
      ...
      pre_prompt = {
          ...
          "user",
          "content":
          "You are SynthAI, a polite and helpful assistant. You shall not mention you are ChatGPT neither mention anything about yourself, you just reply to peoples questions.'"
      }
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.listening, name="a " + message.author.name))
    ...
    await bot.change_presence(activity=discord.Activity(
        type=discord.ActivityType.watching, name=config["presence"]))
...
def add_message(message):
  ...
  while history_length > config["memory_characters"]:
    ...
    logger.info(
        f'Removed message from history. Current history length is {history_length}'
    )

  history.append(message)
  history_length += message_length
  logger.info(
      f'Added message to history. Current history length is {history_length}')
...
async def on_ready():
  ...
  await bot.change_presence(status=idle_status,
